...
int main(){
  ...
  //  system("clear");
    ...
    switch(opt){
        ...
        case 2:
    fp = fopen("RestaurantBill.dat", "rb"); // Open the file in binary read mode
    if (fp == NULL) {
    ...
    while (fread(&order, sizeof(struct orders), 1, fp) == 1) {
        float tot = 0;
       // printf("Checking order for customer: %s\n", order.customer); // Debugging line

        // Use strncmp to compare customer names, allowing for case-insensitive comparison
        if (strncasecmp(order.customer, name, strlen(name)) == 0) {
            generateBillHeader(order.customer, order.date);
            for (int i = 0; i < order.numOfItems; i++) {
                generateBillBody(order.itm[i].item, order.itm[i].qty, order.itm[i].price);
                tot += order.itm[i].qty * order.itm[i].price;
